 ...
 */
	...
	#include "main.h"		// NVPro samples header. Provides nvprintf
	...
	unsigned long getFilePos ( FILE* fp );
		...
		extern bool checkHR( HRESULT hr, std::string func );
		extern bool checkDEV ( std::string func );
	...
	public:
		...
		void SetVert ( int n, float x, float y, float z, float tx, float ty );
		...
		int getNumVerts ()		{ return mVertices.size(); }
