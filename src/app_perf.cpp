...
void PERF_INIT ( int buildbits, bool cpu, bool gpu, bool cons, int lev, char* fname )
{
	...
	Time start;		// create Time obj to initialize system timer
}
...
long Time::GetFracMonth ( Time& base )
{
	// Resolution = 4 hrs
...
long Time::GetFracYear ( Time& base )
{
	// It is much easier to compute this in m/d/y format rather
	// than using julian dates.
	int bhr, bmin, bm, bd, by;
